...
func New(pathDB string) (*Storage, error) {
	if strings.TrimSpace(pathDB) == "" {
...
func (s *Storage) Collect(record model.Record) error {
...
func (s *Storage) LoadCollect() (map[string]model.Record, error) {
	var m = make(map[string]model.Record)
	var rr []model.Record
...
func (s *Storage) NewChat(chat *tba.Chat) error {
...
func (s *Storage) NewMessage(msg *tba.Message) error {
...
func (s *Storage) GetNotUseCoupon(cid int64) ([]model.Record, error) {
	var rr []model.Record
...
func (s *Storage) GetNotUseCouponCount(cid, count int64) ([]model.Record, error) {
	var rr []model.Record
	var t = time.Now().AddDate(0, 0, -1).Unix()
...
func (s *Storage) MarkAsRead(cid int64, rr []model.Record) error {
	for _, r := range rr {
